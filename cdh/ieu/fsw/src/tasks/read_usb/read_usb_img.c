...
void read_usb_img(void) {
    ...
    rt_printf("%d (READ_USB_IMG_TASK) Waiting for filter table task"
        " to be ready\n",time(NULL));
    ...
    // Print:
    rt_printf("%d (READ_USB_IMG_TASK) Filter table task is ready;"
        " continuing\n",time(NULL));
    ...
    // Print:
    rt_printf("%d (READ_USB_IMG_TASK) Ready to process images, interface with"
    ...
    while (1) {
        ...
        // Print:
        rt_printf("%d (READ_USB_IMG_TASK) Reading new raw image\n",time(NULL));
        ...
        if (ret_val == RAW_IMG_SIZE) {
            // Print:
            rt_printf("%d (READ_USB_IMG_TASK) Sent raw image to IPS; waiting"
                " for reply\n",time(NULL));
        } else {
            rt_printf("%d (READ_USB_IMG_TASK) Error sending raw image"
                " to IPS\n",time(NULL));
        ...
        if (ips_ret != 0) {
            // Print:
            rt_printf("%d (READ_USB_IMG_TASK) Image classified to have an"
            ...
            if (ret_val == ips_ret) {
                // Print:
                rt_printf("%d (READ_USB_IMG_TASK) Received processed image"
                    " from IPS\n",time(NULL));
            } else {
                rt_printf("%d (READ_USB_IMG_TASK) Error receiving processed"
                    " image from IPS\n",time(NULL));
                // NEED ERROR HANDLING
            }
            ...
            for (i = 0; i < num_tlm_pkt_xfr_frm_req; ++i) {
                ...
                if ((ret_val > 0) || (ret_val == 0)) {
                    // Print:
                    //rt_printf("%d (READ_USB_IMG_TASK) Telemetry packet transfer"
                    ...
                    // Print:
                    rt_printf("%d (READ_USB_IMG_TASK) Error sending telemetry"
                        " packet transfer frame\n",time(NULL));
                    // NEED ERROR HANDLING
                }
            }
            // Print:
            rt_printf("%d (READ_USB_IMG_TASK) All %d telemetry packet transfer"
                " frames sent to filter table task\n",time(NULL),\
                num_tlm_pkt_xfr_frm_req);
        } else {
            // Print:
            rt_printf("%d (READ_USB_IMG_TASK) Image classified to not have an"
