...
#define CMD_BGNIMGACQ 0x00 // Command: Acquire image
#define CMD_NOOP      0x01 // Command: Non-operational
...
RT_TASK_MCB cmd_xfr_frm_img_mcb; // For command transfer frame message from
                                 // command executor task
RT_TASK_MCB rply_img_mcb;        // For command execution status reply message
...
void cmd_img(void* arg) {
    ...
    uint32_t acq_ivl = 60;                  // Acquisition interval in seconds
    RTIME    acq_ivl_timeout = acq_ivl*1e9; // Acquisition interval timeo
                                            // in seconds
    
    int8_t acq_prog_flag = 0; // Acquisition in progress flag
    ...
    cmd_xfr_frm_img_mcb.data = cmd_xfr_frm_buf;  // Set message block buffer
    cmd_xfr_frm_img_mcb.size = CMD_XFR_FRM_SIZE; // Set message block buffer
                                                 // size

    rply_img_mcb.data = &cmd_exec_stat; // Set reply message buffer
    rply_img_mcb.size = RPLY_MSG_SIZE;  // Set reply message size

    // Initialize camera:
    init_cam(); // Set exposure
    ...
    while (1) {
        ...
        // synchronous message:
        flw_id = rt_task_receive(&cmd_xfr_frm_img_mcb,TM_INFINITE);
        ...
        switch (cmd_pkt_name) {
            ...
            case CMD_BGNIMGACQ :
                ...
                if (acq_prog_flag == 0) {
                    // Print:
                    rt_printf("%d (CMD_IMG_TASK) Executing ACQIMG command with"
                        " duration %d seconds\n",time(NULL),cmd_arg);

                    // Set flag:
                    acq_prog_flag = 1; // Acquisition is in progress
                    ...
                    // in progress:
                    ret_val = rt_task_reply(flw_id,&rply_img_mcb);
                    ...
                    while (elp_time < acq_dur) {
                        ...
                        // timeout interval:
                        flw_id = rt_task_receive(&cmd_xfr_frm_img_mcb,\
                            acq_ivl_timeout);
                        ...
                        // (This increment will only occur if a command was
                        // not received during acquisition interval timeout)
                        elp_time =+ acq_ivl;
                    }

                    // Set flag:
                    acq_prog_flag = 0; // Acquisition is not in progress*/
                // Currently in progress
                } else {
                    // Print:
                    rt_printf("%d (CMD_IMG_TASK) Image acquisition already in"
                        " progress; ignoring command\n",time(NULL));
