...
void cmd_img(void* arg) {
    ...
    cmd_xfr_frm_sw_mcb.data = cmd_xfr_frm_buf;  // Set message block buffer
    cmd_xfr_frm_sw_mcb.size = CMD_XFR_FRM_SIZE; // Set message block buffer
                                                // size
    ...
    while(1) {
        ...
        // synchronous message:
        flw_id = rt_task_receive(&cmd_xfr_frm_sw_mcb,\
        ...
        switch (cmd_pkt_name) {
            ...
            default :
                // Print:
                rt_printf("%d (CMD_IMG_TASK) Command packet is invalid;" 
