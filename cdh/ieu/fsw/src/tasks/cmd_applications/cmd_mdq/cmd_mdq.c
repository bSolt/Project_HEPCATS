...
RT_TASK_MCB cmd_xfr_frm_mdq_mcb; // For command transfer frame message from
                                 // command executor task
RT_TASK_MCB rply_sw_mcb;         // For command execution status reply message
...
void cmd_mdq(void* arg) {
    ...
    cmd_xfr_frm_mdq_mcb.data = cmd_xfr_frm_buf;  // Set message block buffer
    cmd_xfr_frm_mdq_mcb.size = CMD_XFR_FRM_SIZE; // Set message block buffer
                                                 // size
    ...
    while(1) {
        ...
        // synchronous message:
        flw_id = rt_task_receive(&cmd_xfr_frm_mdq_mcb,\
        ...
        ret_val = rt_task_reply(flw_id,&rply_sw_mcb);
