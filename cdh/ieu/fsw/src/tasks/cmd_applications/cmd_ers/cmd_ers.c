...
RT_TASK_MCB cmd_xfr_frm_ers_mcb; // For command transfer frame message from
                                 // command executor task
RT_TASK_MCB rply_ers_mcb;        // For command execution status reply message
...
void cmd_ers(void* arg) {
    ...
    cmd_xfr_frm_ers_mcb.data = cmd_xfr_frm_buf;  // Set message block buffer
    cmd_xfr_frm_ers_mcb.size = CMD_XFR_FRM_SIZE; // Set message block buffer
                                                 // size

    rply_ers_mcb.data = &cmd_exec_stat; // Set reply message buffer
    rply_ers_mcb.size = RPLY_MSG_SIZE;  // Set reply message size
    ...
    while(1) {
        ...
        // synchronous message:
        flw_id = rt_task_receive(&cmd_xfr_frm_ers_mcb,\
        ...
        ret_val = rt_task_reply(flw_id,&rply_ers_mcb);
