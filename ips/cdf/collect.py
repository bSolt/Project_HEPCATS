# This will be a python module for collecting spacecraft data in cdf format and saving it to a single pickle file
...
for file in cdf_list:
	if '.cdf' in file:
		...
		while(i<len(cdf[EP])):
			add_to_data(cdf,i)
			t0 = cdf[EP][i]
			# skip all the images which are taken less than some time (DELAY) later the previous
...
print(f'Data from cdfs in {cdf_dir} saved to {outname}\nTotal images: {len(data["Image"])}')